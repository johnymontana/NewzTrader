...
def index():
   ...
   # newsCats = []
   # nb_classifier = get_classifier()
    
   # for headline in todaysNews:
   #     newsCats.append(nb_classifier.classify(bag_of_words(headline)))
    
    return dict(news=todaysNews)

def get_classifier():

    fp = os.path.join(request.folder,'private','classifier.pkl')
    pkl_file = open(fp, 'rb')
...
def get_headlines(newsDate):
    ...
    for element in myRoot.iter("article"):
        ...
        news.append(only_alphanum(removeNonAscii(title)))
    return news
